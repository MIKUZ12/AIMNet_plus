...
def loadMfDIMvMlDataFromMat(mat_path, fold_mat_path,fold_idx=0):
    ...
    ## 已经经过分割后的label，相当于根据train，val，test的分割重新排序了
    inc_labels = labels
    ## 掩码之后的label
...
class IncDataset(Dataset):
    def __init__(self,mat_path, fold_mat_path, training_ratio=0.7, val_ratio=0.15, fold_idx=0, mode='train',semisup=False):
        ...
        ## inc_V_ind 和 inc_L_ind : inc_view_indicator,inc_label_indicator
        if mode == 'train':
            ...
            self.cur_labels = inc_labels[:self.train_sample_num]* self.cur_inc_L_ind # 经过掩码之后的label
        elif mode == 'val':
