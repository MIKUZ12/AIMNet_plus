...
def train(loader, model, loss_model, opt, sche, epoch,dep_graph,last_preds,logger):
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        # data是多视图的信息
        z_sample, uniview_mu_list, uniview_sca_list, fusion_z_mu, fusion_z_sca, xr_list, label_emb_sample, pred, label_emb,label_emb_var,p_xr_list, xr_s_list, xr_p_list, pos_beta_I = model(data,mask=inc_V_ind )
        ...
        if epoch<args.pre_epochs:
            ...
            for v in range(len(data)):
                ## xr_list是每个重构的视图，这个损失是用来约束VAE的，使得潜在空间的特征能够很好的表征原数据
                loss_mse_s += loss_model.weighted_wmse_loss(data[v],xr_s_list[v],inc_V_ind[:,v],reduction='mean')
            for v in range(len(data)):
                ## xr_list是每个重构的视图，这个损失是用来约束VAE的，使得潜在空间的特征能够很好的表征原数据
                loss_mse_p += loss_model.weighted_wmse_loss(data[v],xr_p_list[v],inc_V_ind[:,v],reduction='mean')
            
            loss_pos_beta_I = pos_beta_I
            
            assert torch.sum(torch.isnan(loss_mse)).item() == 0
            loss = loss_CL + loss_mse *args.alpha + z_c_loss*args.beta + cohr_loss *args.sigma + loss_mse_s + loss_mse_p + loss_pos_beta_I
        # loss = loss_CL
        opt.zero_grad()
        loss.backward()
...
def test(loader, model, loss_model, epoch,logger):
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        z_sample, uniview_mu_list, uniview_sca_list, fusion_z_mu, fusion_z_sca, xr_list, label_embedding_sample, qc_z, label_emb,label_emb_var, xr_list_views, xr_s_list, xr_p_list, pos_beta_I = model(data,mask=inc_V_ind.to('cuda:0'))
        ...
        for label_missing_rate in args.label_missing_rates:
            for feature_missing_rate in args.feature_missing_rates:
                ...
                for fold_idx in range(folds_num):
                    ...
                    folder_name = f'E:/desktop/AIMNet/Index/{dataset_name}'
